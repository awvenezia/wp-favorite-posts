...
if ( isset($_REQUEST['nonce']) && wp_verify_nonce( sanitize_key($_REQUEST['nonce']), "submit_nonce") && isset($_POST['submit'])) {
    ...
    if (isset($_REQUEST['show_remove_link']) && $_REQUEST['show_remove_link'] == 'show_remove_link')
        $_REQUEST['added'] = 'show remove link';

    if (isset($_REQUEST['show_add_link']) && $_REQUEST['show_add_link'] == 'show_add_link')
        $_REQUEST['removed'] = 'show add link';

	$wpfp_options['add_favorite'] = isset($_REQUEST['add_favorite']) ? htmlspecialchars(sanitize_text_field($_REQUEST['add_favorite'])) : null;
	$wpfp_options['added'] = isset($_REQUEST['added']) ? htmlspecialchars(sanitize_text_field($_REQUEST['added'])) : null;
	$wpfp_options['remove_favorite'] = isset($_REQUEST['remove_favorite']) ?  htmlspecialchars(sanitize_text_field($_REQUEST['remove_favorite'])) : null;
	$wpfp_options['removed'] = isset($_REQUEST['removed']) ?  htmlspecialchars(sanitize_text_field($_REQUEST['removed'])) : null;
	$wpfp_options['clear'] = isset($_REQUEST['clear']) ?  htmlspecialchars(sanitize_text_field($_REQUEST['clear'])) : null;
	$wpfp_options['cleared'] = isset($_REQUEST['cleared']) ?  htmlspecialchars(sanitize_text_field($_REQUEST['cleared'])) : null;
	$wpfp_options['favorites_empty'] = isset($_REQUEST['favorites_empty']) ?  htmlspecialchars(sanitize_text_field($_REQUEST['favorites_empty'])) : null;
	$wpfp_options['rem'] = isset($_REQUEST['rem']) ?  htmlspecialchars(sanitize_text_field($_REQUEST['rem'])) : null;
	$wpfp_options['cookie_warning'] = isset($_REQUEST['cookie_warning']) ?  htmlspecialchars(sanitize_text_field($_REQUEST['cookie_warning'])) : null;
	$wpfp_options['text_only_registered'] = isset($_REQUEST['text_only_registered']) ?  htmlspecialchars(sanitize_text_field($_REQUEST['text_only_registered'])) : null;
	$wpfp_options['statistics'] = isset($_REQUEST['statistics']) ?  htmlspecialchars(sanitize_text_field($_REQUEST['statistics'])) : null;
	$wpfp_options['before_image'] = isset($_REQUEST['before_image']) ?  htmlspecialchars(sanitize_text_field($_REQUEST['before_image'])) : null;
	$wpfp_options['custom_before_image'] = isset($_REQUEST['custom_before_image']) ?  htmlspecialchars(sanitize_text_field($_REQUEST['custom_before_image'])) : null;
	$wpfp_options['autoshow'] = isset($_REQUEST['autoshow']) ?  htmlspecialchars(sanitize_text_field($_REQUEST['autoshow'])) : null;
	$wpfp_options['post_per_page'] = isset($_REQUEST['post_per_page']) ?  htmlspecialchars(sanitize_text_field($_REQUEST['post_per_page'])) : null;

    $wpfp_options['dont_load_js_file'] = '';
    if (isset($_REQUEST['dont_load_js_file']))
        $wpfp_options['dont_load_js_file'] = htmlspecialchars(sanitize_text_field($_REQUEST['dont_load_js_file']));

    $wpfp_options['dont_load_css_file'] = '';
    if (isset($_REQUEST['dont_load_css_file']))
        $wpfp_options['dont_load_css_file'] = htmlspecialchars(sanitize_text_field($_REQUEST['dont_load_css_file']));

    $wpfp_options['opt_only_registered'] = '';
    if (isset($_REQUEST['opt_only_registered']))
        $wpfp_options['opt_only_registered'] = htmlspecialchars(sanitize_text_field($_REQUEST['opt_only_registered']));
...
if ( isset($_GET['action'] ) ) {
	if ($_GET['action'] == 'reset-statistics') {
		...
		if ($wpdb->query($wpdb->prepare("DELETE FROM $wpdb->postmeta WHERE meta_key = %s", 'wpfp_favorites'))) {
...
<?php $nonce = wp_create_nonce("submit_nonce"); ?>
        ...
        <table class="form-table">
            ...
            <tr>
                ...
                <td>
                    ...
                    ?>
                    <label for="<?php echo wp_kses_post($img) ?>">
                        <input type="radio" name="before_image" id="<?php echo esc_attr($img) ?>" value="<?php echo wp_kses_post($img) ?>" <?php if ($wpfp_options['before_image'] == $img) echo "checked='checked'" ?> />
                        <img src="<?php echo wp_kses_post(WPFP_PATH); ?>/img/<?php echo wp_kses_post($img); ?>" alt="<?php echo wp_kses_post($img); ?>" title="<?php echo esc_attr($img); ?>" class="wpfp-img" />
                    </label>
                    <br />
                    <?php
                    endforeach;
                    ...
                    <input type="custom_before_image" name="custom_before_image" value="<?php echo wp_kses_post(stripslashes($wpfp_options['custom_before_image'])); ?>" />
                    <br />
                    <label for="none">
            ...
            <tr>
                ...
                <td>
                    <input type="text" name="post_per_page" size="2" value="<?php echo wp_kses_post(stripslashes($wpfp_options['post_per_page'])); ?>" /> * This only works with default favorite post list page (wpfp-page-template.php).
                </td>
            </tr>
            <tr>
...
<div class="postbox">
    ...
    <div class="inside" style="display: block;">
        ...
        <table class="form-table">
            <tr>
                <th><?php _e("Text for add link", "wp-favorite-posts") ?></th><td><input type="text" name="add_favorite" value="<?php echo wp_kses_post(stripslashes($wpfp_options['add_favorite'])); ?>" /></td>
            </tr>
            <tr>
				...
				<br /><input id="added" type="text" name="added" <?php if ($wpfp_options['added'] == 'show remove link') echo "style='display:none;'"; ?> value="<?php echo wp_kses_post(stripslashes($wpfp_options['added'])); ?>" /></td>
            </tr>
            <tr>
                <th><?php _e("Text for remove link", "wp-favorite-posts") ?></th><td><input type="text" name="remove_favorite" value="<?php echo wp_kses_post(stripslashes($wpfp_options['remove_favorite'])); ?>" /></td>
            </tr>
            <tr>
				...
				<br />
				<input id="removed" type="text" name="removed" <?php if ($wpfp_options['removed'] == 'show add link') echo "style='display:none;'"; ?> value="<?php echo wp_kses_post(stripslashes($wpfp_options['removed'])); ?>" /></td>
            </tr>
            <tr>
                <th><?php _e("Text for clear link", "wp-favorite-posts") ?></th><td><input type="text" name="clear" value="<?php echo wp_kses_post(stripslashes($wpfp_options['clear'])); ?>" /></td>
            </tr>
            <tr>
                <th><?php _e("Text for cleared", "wp-favorite-posts") ?></th><td><input type="text" name="cleared" value="<?php echo wp_kses_post(stripslashes($wpfp_options['cleared'])); ?>" /></td>
            </tr>
            <tr>
                <th><?php _e("Text for favorites are empty", "wp-favorite-posts") ?></th><td><input type="text" name="favorites_empty" value="<?php echo wp_kses_post(stripslashes($wpfp_options['favorites_empty'])); ?>" /></td>
            </tr>
            <tr>
                <th><?php _e("Text for [remove] link", "wp-favorite-posts") ?></th><td><input type="text" name="rem" value="<?php echo wp_kses_post(stripslashes($wpfp_options['rem'])); ?>" /></td>
            </tr>
            <tr>
                <th><?php _e("Text for favorites saved to cookies", "wp-favorite-posts") ?></th><td><textarea name="cookie_warning" rows="3" cols="35"><?php echo wp_kses_post(stripslashes($wpfp_options['cookie_warning'])); ?></textarea></td>
            </tr>
            <tr>
                <th><?php _e("Text for \"only registered users can favorite\" error message", "wp-favorite-posts") ?></th><td><textarea name="text_only_registered" rows="2" cols="35"><?php echo wp_kses_post(stripslashes($wpfp_options['text_only_registered'])); ?></textarea></td>
            </tr>
